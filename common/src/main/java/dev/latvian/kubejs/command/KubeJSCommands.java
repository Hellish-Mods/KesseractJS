...
public class KubeJSCommands {
    public static final Predicate<CommandSourceStack>
        SINGLE_PLAYER_OR_OP = (source) -> source.getServer().isSingleplayer() || source.hasPermission(2);

    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("kubejs")
            .then(Commands.literal("custom_command")
                .then(Commands.argument("id", StringArgumentType.word())
                    .executes(context -> customCommand(
                        context.getSource(),
...
//                    .requires(source -> true)
            ...
            .then(Commands.literal("painter")
                .then(Commands.argument("player", EntityArgument.players())
                    .then(Commands.argument("object", CompoundTagArgument.compoundTag())
                        .executes(context -> painter(
                            context.getSource(),
                            EntityArgument.getPlayers(context, "player"),
                            CompoundTagArgument.getCompoundTag(context, "object")
                        ))
                    )
                )
            )
    ...
    private static Component copy(String s, ChatFormatting format, String info) {
         return Text.of("- ")
            .color(ColorWrapper.GRAY)
            .click(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, s))
            .hover(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponent(info + " (Click to copy)")))
            .append(new TextComponent(s).withStyle(format))
            .component();
    }

    private static int customCommand(CommandSourceStack source, String id) {
        new CustomCommandEventJS(source.getLevel(), source.getEntity(), new BlockPos(source.getPosition()), id).post(
            ScriptType.SERVER,
            KubeJSEvents.SERVER_CUSTOM_COMMAND,
            id
        );
        return 1;
    }
    ...
    private static int hand(ServerPlayer player, InteractionHand hand) {
        ...
        for (ResourceLocation id : tags) {
            player.sendMessage(copy(
                ...
                "Item Tag [" + TagIngredientJS.createTag(id.toString()).getStacks().size() + " items]"
            ), Util.NIL_UUID);
        }
        ...
        player.sendMessage(copy(
            "'@" + stack.getMod() + "'",
            ChatFormatting.AQUA,
            "Mod [" + new ModIngredientJS(stack.getMod()).getStacks().size() + " items]"
        ), Util.NIL_UUID);
        ...
        if (stack.getItem().getItemCategory() != null) {
            player.sendMessage(copy(
                ...
                "Item Group [" + new GroupIngredientJS(stack.getItem().getItemCategory()).getStacks().size() + " items]"
            ), Util.NIL_UUID);
        }
        ...
        if (ScriptType.SERVER.errors.isEmpty()) {
            ...
            if (!ScriptType.SERVER.warnings.isEmpty()) {
                source.sendSuccess(new TextComponent(
                    ScriptType.SERVER.warnings.size() + " warnings found. Run /kubejs warnings to see them").withStyle(
                    Style.EMPTY.withColor(TextColor.fromRgb(0xFFA500))), false);
            }
            return 1;
        }

        for (int i = 0; i < ScriptType.SERVER.errors.size(); i++) {
            source.sendSuccess(new TextComponent("[" + (i + 1) + "] " + ScriptType.SERVER.errors.get(i)).withStyle(
                ChatFormatting.RED), false);
        }

        source.sendSuccess(
            new TextComponent("More info in 'logs/kubejs/server.txt'").withStyle(ChatFormatting.DARK_RED),
            false
        );
    ...
    private static int warnings(CommandSourceStack source) {
        ...
        for (int i = 0; i < ScriptType.SERVER.warnings.size(); i++) {
            source.sendSuccess(new TextComponent("[" + (i + 1) + "] " + ScriptType.SERVER.warnings.get(i)).withStyle(
                Style.EMPTY.withColor(TextColor.fromRgb(0xFFA500))), false);
        }
    ...
    private static <T> int tagObjects(ServerPlayer player,
        TagCollection<T> collection,
        ResourceKey<Registry<T>> reg,
        ResourceLocation t
    ) {
    ...
    private static int wiki(CommandSourceStack source) {
        source.sendSuccess(
            Text
                .of("Click here to open the Wiki")
                .color(ColorWrapper.BLUE)
                .click(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://kubejs.com/"))
                .component(),
            false
        );
        return Command.SINGLE_SUCCESS;
    ...
    private static int addStage(CommandSourceStack source, Collection<ServerPlayer> players, String stage) {
        for (ServerPlayer p : players) {
            if (Stages.get(p).add(stage)) {
                source.sendSuccess(
                    new TextComponent("Added '" + stage + "' stage for " + p.getScoreboardName()),
                    false
                );
            }
        }
    ...
    private static int removeStage(CommandSourceStack source, Collection<ServerPlayer> players, String stage) {
        for (ServerPlayer p : players) {
            if (Stages.get(p).remove(stage)) {
                source.sendSuccess(
                    new TextComponent("Removed '" + stage + "' stage for " + p.getScoreboardName()),
                    false
                );
            }
        }
    ...
    private static int listStages(CommandSourceStack source, Collection<ServerPlayer> players) {
        for (ServerPlayer p : players) {
            ...
            Stages.get(p)
                ...
                .stream()
                .sorted()
                .forEach(s -> source.sendSuccess(new TextComponent("- " + s), false));
        }
        return Command.SINGLE_SUCCESS;
    }
